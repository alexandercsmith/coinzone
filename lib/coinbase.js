...
module.exports = class Coinbase {
  ...
  constructor ({ key, phrase, secret, sandbox=true }) {
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log('>', 'initializing coinbase interface:', 
  ...
  */
  ...
  /**
   * @async @function profileDefault
   * @return { Object }
   */
  profileDefault = async () => await this.get('/profiles', params)
    .then(profiles => profiles.find(profile.is_default));
  ...
  */
   ...
   * @return { Object }
   */
  async cancel (id, auth=false) {
    try {
      return await this.api.request({
        url:    '/orders' + "/".concat(id) + (!!auth ? "client:".concat(this.clientId) : ''),
        method: 'delete'
      }).then(res => res.data);
    } catch (e) {
      console.error('coinbase: cancel', e.message);
      throw new Error(e);
    }
  }
  ...
  async cancelAll () {
    try {
      ...
      console.error('coinbase: cancelAll', e.message);
      throw new Error(e);
    }
  }
  ...
  /**
   * @function _toBuyOrder
   * @param  { Object } input
   * @return { Object }
   */
  _toBuyOrder (input={ size, price, product_id, stop_price }) {
    return { 
      ...input,
      client_oid: this.clientId,
      type:       "limit",
      side:       "buy"
    }
  }
  ...
  /**
   * @function _toSellOrder
   * @param  { Object } input
   * @return { Object }
   */
  _toSellOrder (input={ size, price, product_id, stop_price }) {
    return { 
      ...input,
      client_oid: this.clientId,
      type:       "limit",
